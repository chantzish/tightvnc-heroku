...
#kernel compilation
    ...
    sudo bash -c -O extglob "chown -R `id -un`:`id -gn` /"'!(dev|etc|proc|sys|tmp) /etc/!(bash.bashrc|group|hostname|hosts|passwd|profile|resolv.conf) && chown '"`id -un`:`id -gn` / /tmp /etc" && \
