...
#kernel compilation
    ...
    echo 1234 | sudo -S bash -c -O extglob 'chgrp -R 0 /!(sys|proc|etc|run|usr) /etc/!(resolv.conf|sudoers|sudoers.d|mtab) /run/!(sudo) /usr/!(bin|lib) /usr/bin/!(sudo) /usr/lib/!(sudo) && chgrp 0 / /etc /usr /run /usr/bin /usr/lib' && \
    echo 1234 | sudo -S bash -c -O extglob 'chmod -R g+u /!(sys|proc|etc|run|usr) /etc/!(resolv.conf|sudoers|sudoers.d|mtab) /run/!(sudo) /usr/!(bin|lib) /usr/bin/!(sudo) /usr/lib/!(sudo) && chmod g+u / /etc /usr /run /usr/bin /usr/lib'
