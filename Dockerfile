...
#kernel compilation
    ...
    sudo bash -c -O extglob "chown -R `id -un`:`id -gn` /"'!(dev|etc|proc|sys|tmp|usr) /etc/!(bash.bashrc|group|hostname|hosts|passwd|profile|resolv.conf) /usr/!(bin|lib) /usr/bin/!(sudo) /usr/lib/!(sudo) && chown '"`id -un`:`id -gn` / /tmp /etc" && \
