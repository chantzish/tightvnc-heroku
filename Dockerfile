...
#kernel compilation
    ...
    sudo bash -c -O extglob "chown -R `id -un`:`id -gn` /"'!(dev|etc|proc|sys|tmp|usr) /etc/!(bash.bashrc|group|hostname|hosts|passwd|profile|resolv.conf) /usr/!(bin) /usr/bin/!(sudo) && chown '"`id -un`:`id -gn` / /tmp /etc" && \
